...
impl LineStreamer {
    ...
    fn buffer_next(&mut self) {
        /*
        Advances the internal buffer to the next valid one. If there isn't one then the buffer is
        set to an error.
        */
        ...
        while self.buffer.is_err() && self.entries_ctr < self.entries.len() {
            ...
            match File::open(entry.path()) {
                Ok(f) => {
                    self.buffer = Ok(BufReader::new(f));
                }
    ...
    fn line_next(&mut self) {
        /*
        Advances the internal line to the next valid line. If there isn't one then the line is set
        to an error.
        */
        // If the buffer is uninitialized then initialize it.
        if self.buffer.is_err() {
            self.buffer_next();
        }
